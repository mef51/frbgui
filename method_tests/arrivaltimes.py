...
def gaussian_model(x, a, xo, sigma):
...
def fitgaussian(data, duration):
	...
	popt, pcov = scipy.optimize.curve_fit(gaussian_model, x, data, p0=guess)
	return popt, pcov
...
if __name__ == '__main__':
	...
	for filename in files:
		...
		# t_popt, _ = fitgaussian(tseries, duration)
		# window = abs(t_popt[2])
		window = np.var(tseries) # use the variance as a window even though the units dont make sense, cuz it works

		arrtimesdf = arrtimesdf[(pktime-window < arrtimesdf.xo) & (arrtimesdf.xo < pktime+window)]
		...
		results.append([bname.split('burst_')[1], data['DM'], dtdnu_start, dtdnu_start_err])
		...
		ax_wfall.scatter(arrtimesdf[tpoint]-xobar, arrtimesdf['freqs'], c=pt_colors, marker='o', s=25)
		ax_wfall.set_xlabel("Time (ms)")
		ax_wfall.set_ylabel("Frequency (MHz)")
		ax_wfall.plot(times*1000-xobar, (1/dtdnu_start)*(times*1000 - xobar), 'w--', alpha=0.75)
